...
class PaymentProcessorService {
    constructor() {
        this.logger = log4js_1.getLogger(constants_1.LoggingCategory.PAYMENT_PROCESSING);
    }
    async ProcessEMVSRED(payinfo) {
        /*eslint-disable */
        return new Promise(async (resolve) => {
            ...
            const processEMVSREDRequests = payinfo.map((item) => {
                return `<mpp1:ProcessEMVSREDRequest>
                   ...
                   <mpp1:CustomerTransactionID>${item.CustomerTransactionID}</mpp1:CustomerTransactionID>
                   <mpp1:EMVSREDInput>
                      ...
                      <mpp1:NumberOfPaddedBytes>${item.EMVSREDInput.NumberOfPaddedBytes}</mpp1:NumberOfPaddedBytes>
                   ...
                   <mpp1:TransactionInput>
                      ...
                      <mpp1:TransactionInputDetails>
                         <sys:KeyValuePairOfstringstring>
                             <sys:key>${item.TransactionInput.TransactionInputDetails
                    .KeyValuePairOfstringstring.key}</sys:key>
                             <sys:value>${item.TransactionInput.TransactionInputDetails
                    .KeyValuePairOfstringstring.value}</sys:value>
                         </sys:KeyValuePairOfstringstring>
                      </mpp1:TransactionInputDetails>
                      <mpp1:TransactionType>${item.TransactionInput.TransactionType}</mpp1:TransactionType>
                   </mpp1:TransactionInput>
        ...
        <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:mpp="http://www.magensa.net/MPPGv3/" xmlns:mpp1="http://schemas.datacontract.org/2004/07/MPPGv3WS.Core" xmlns:sys="http://schemas.datacontract.org/2004/07/System.Collections.Generic">
           ...
           <soapenv:Body>
            ...
            if (statusCode === 200) {
                ...
                this.logger.debug('response JSON', responseJson.Body.ProcessEMVSREDResponse.ProcessEMVSREDResult.ProcessEMVSREDResponse);
                const ProcessEMVSREDResponse = responseJson.Body.ProcessEMVSREDResponse.ProcessEMVSREDResult.ProcessEMVSREDResponse;
                resolve({
                    success: true,
                ...
                this.logger.error('Error processing payment. Status code:', statusCode, 'response object:', response);
